...
function resizeDragArea() {
  ...
  $('.ar-unscramble-container').css(
    'min-height',
    (($dropArea.innerHeight() * 55) / 45) * 2
  )
}
...
drag.fallout
  ...
  .on('move', function(token, srcNode, info) {
    ...
    if (dropPosition >= 0) {
      let onlyOneAndLeanLeft =
        ...
        DragPolar.getTargetInfo(token).target_X -
          DragPolar.getTargetInfo($labelsInDropArea[dropPosition]).target_X <=
          0
      ...
      if (onlyOneAndLeanLeft) {
        ...
        !dragFromDrop &&
          $guidePhantom.insertBefore(
            $labelsInDropArea.eq(dropPosition).parent()
          )
      }
      // append after
        ...
        // $guidePhantom = $(srcNode).parent().clone();
        !dragFromDrop &&
          $guidePhantom.insertAfter($labelsInDropArea.eq(dropPosition).parent())
      }
    }
  })
  .on('drop', function(token, srcNode) {
    ...
    if (dropPosition === -2) {
      ...
      DragPolar.moveItem(
        token,
        getDestCordByStart($guideNode[0]),
        speed,
        function() {
          resetChanges(token)
          // remove code below in angular
    ...
    if (dropPosition >= -1) {
      ...
      !dragFromDrop &&
        DragPolar.moveItem(
          token,
          getDestCordByStart($guidePhantom[0]),
          speed,
          function() {
            resetChanges(token)
...
function getDropPosition(token, srcNode) {
  ...
  if (DragPolar.detectTouch(token, $dropArea[0])) {
    ...
    $labelsInDropArea.each(function(idx) {
      if (DragPolar.detectTouch(token, this))
        touchList.push([DragPolar.distanceBetween(token, this), idx])
    })
